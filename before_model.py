...
def split_sdf(input_dir: str, output_dir: str, split_keyword: str):
    ...
    for idx, mol in enumerate(supplier):
        ...
        writer = SDWriter(os.path.join(output_dir, f"{mol_id}.sdf"))
        writer.write(mol)
        writer.close()


def generate_model_input(protein_dir: str, split_ligands_dir: str, model_inputs_dir: str, num_chunks: int):
    protein_id = os.listdir(protein_dir)[0].split(".")[0]
...
def get_commands(task_id: str):
    task_dir = os.path.join(CODE_ROOT_DIR_NAME, task_id)
    all_commands = []
    for index in range(len(os.listdir(os.path.join(ALL_TASKS_DIR, task_id, "all_model_inputs")))):
        current_command = ""
        current_command += f"CUDA_VISIBLE_DEVICES={GPU_INDEXES[index]} nohup python -m inference "
